...
data.ovary<-read.table("data/ovary.germlineClust1.gene.txt",row.names = 1,header = T,sep = "\t",quote="\"")
...
data.ovary.other<-subset(data.ovary,!(rownames(data.ovary) %in% data.out$name))
write.table(data.out,file="data/gProfiler.piRNA_hc.ovaryGene.txt",quote = F,sep="\t")
write.table(data.ovary.other,file="data/gProfiler.piRNA_hc.ovaryGene.negCtrl.txt",quote = F,sep="\t")
